...
public class ViewAsignatura extends JFrame {
    ...
    public ViewAsignatura() {
        ...
        buscarButtonAsig.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                try {
                    String nombre = textNombreAsig.getText().trim();
                    String tipo = textTipoAsig.getText().trim();
                    String curso = textCursoAsig.getText().trim();
                    String cuatrimestre = textCuatrimestreAsig.getText().trim();
                    String idProfesor = textIdProfeAsig.getText().trim();
                    String idGrado = textIdGradoAsig.getText().trim();

                    tablaAsignatura.setModel(controller.buscar(nombre, tipo, curso, cuatrimestre, idProfesor, idGrado));
                } catch (SQLException ex) {
                    throw new RuntimeException(ex);
                }
            }
